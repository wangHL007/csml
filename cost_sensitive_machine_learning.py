...
class CostSensitiveMachineLearning(object):
    ...
    def __init__(self, x_train: pd.DataFrame, y_train: pd.Series, x_test: pd.DataFrame, y_test: pd.Series,
        ...
        样本训练集和测试集的划分，
        :param x_train: 样本特征
        :param y_train:两分类的样本标签， 0是安全，1是危险。 y的索引必须与x的索引一致, 值较小的为安全
    ...
    def plot_plans(self, ax=None):
        plans = self.plans_selected
        thresh = self.plans_thresholds
        if ax is None:
            fig, ax = plt.subplots()
        x = np.linspace(0, 1, 6)
        plans_y = []
...
def rock_assessment(target='data2'):
    data = pd.read_excel(r'E:\王洪磊\云空间\代价敏感\数据\冲击鉴定.xlsx')
...
def rock_burst_data(target='data1'):
    ...
    :return:
    """
    data = pd.read_excel(r'E:\王洪磊\云空间\代价敏感\数据\冲击预警-冯.xlsx')
    X = data.iloc[:, 0:6]
    y = data[target]
    return X, y
...
def experiment_data(file_num=None):
    ...
    AE_folder = Path(r'E:\王洪磊\云空间\代价敏感\数据\100ms')
...
def leave_one():
    ...
    PLANs = np.array([[0, 100], [2, 60], [40, 40], [20, 30], [8, 40]])
    AE_folder = Path(r'E:\王洪磊\云空间\代价敏感\数据\100ms')
    save_folder = Path(r'E:\王洪磊\云空间\代价敏感\数据\100ms')
...
def avrage_ten(PLANs, i, save=None):
    ...
    plan_ax = plot_plans(csml.plans_selected, csml.plans_thresholds)
    plan_fig = plan_ax.figure
